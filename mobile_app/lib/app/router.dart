...
class _OrderPageState extends State<OrderPage> {
        ...
        child: Column(children: [
            ...
            child: Column(children: [
              ...
              Container(
                ...
                child: Wrap(spacing: 8, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
                  TDButton(text: '复制订货数量', size: TDButtonSize.small, type: TDButtonType.outline, onTap: _copyOrderColumn),
                  TDButton(text: '导入Excel', size: TDButtonSize.small, type: TDButtonType.outline, theme: TDButtonTheme.primary, onTap: _importExcel),
                  TDButton(
                    ...
                    onTap: () {
                      ...
                      if (!isSorted) {
                        _originalView = List<Map<String, dynamic>>.from(viewItems);
                        _applySortOrder();
                        isSorted = true;
                        setState(() {});
                        TDToast.showText('已按排序顺序', context: context);
                      } else {
                        viewItems = List<Map<String, dynamic>>.from(_originalView);
                        isSorted = false;
                        setState(() {});
                  ...
                  TDButton(text: '粘贴排序文本', size: TDButtonSize.small, type: TDButtonType.outline, theme: TDButtonTheme.primary, onTap: _editSortText),
                  TDButton(text: '上传排序文件', size: TDButtonSize.small, type: TDButtonType.text, onTap: _uploadSortFile),
                  TDButton(text: saving ? '保存中...' : '保存数据', size: TDButtonSize.small, type: TDButtonType.fill, theme: TDButtonTheme.primary, onTap: saving ? null : _saveOrder),
                  SizedBox(
                    width: 180,
                    child: Text(statusText, textAlign: TextAlign.right, style: const TextStyle(color: Colors.grey)),
                  ),
                ]),
              ),
              const SizedBox(height: 8),
              Container(
                ...
                if (dateMode == 0) ...[
                  SizedBox(width: 200, child: TDInput(controller: chooseDateCtrl, hintText: '查询日期(YYYY-MM-DD)')),
                  const SizedBox(width: 8),
                  TDButton(text: '选择日期', size: TDButtonSize.small, type: TDButtonType.outline, theme: TDButtonTheme.primary, onTap: () async {
                    ...
                    if (picked != null) {
                      chooseDateCtrl.text = '${picked.year.toString().padLeft(4, '0')}-${picked.month.toString().padLeft(2, '0')}-${picked.day.toString().padLeft(2, '0')}';
                    }
                  }),
                  const SizedBox(width: 8),
                  TDButton(text: '查询', size: TDButtonSize.small, type: TDButtonType.outline, onTap: _loadByDate),
                  const SizedBox(width: 8),
                  TDButton(text: '保存到日期', size: TDButtonSize.small, type: TDButtonType.outline, onTap: _saveToDate),
                ] else ...[
                  SizedBox(width: 200, child: TDInput(controller: startDateCtrl, hintText: '起始(YYYY-MM-DD)')),
                  const SizedBox(width: 8),
                  TDButton(text: '选择起始', size: TDButtonSize.small, type: TDButtonType.outline, theme: TDButtonTheme.primary, onTap: () async {
                    final now = DateTime.now();
                    final picked = await showDatePicker(context: context, initialDate: now, firstDate: DateTime(now.year - 5), lastDate: DateTime(now.year + 5));
                    if (picked != null) {
                      startDateCtrl.text = '${picked.year.toString().padLeft(4, '0')}-${picked.month.toString().padLeft(2, '0')}-${picked.day.toString().padLeft(2, '0')}';
                    }
                  }),
                  const SizedBox(width: 8),
                  SizedBox(width: 200, child: TDInput(controller: endDateCtrl, hintText: '终止(YYYY-MM-DD)')),
                  const SizedBox(width: 8),
                  TDButton(text: '选择终止', size: TDButtonSize.small, type: TDButtonType.outline, theme: TDButtonTheme.primary, onTap: () async {
                    final now = DateTime.now();
                    final picked = await showDatePicker(context: context, initialDate: now, firstDate: DateTime(now.year - 5), lastDate: DateTime(now.year + 5));
                    if (picked != null) {
                      endDateCtrl.text = '${picked.year.toString().padLeft(4, '0')}-${picked.month.toString().padLeft(2, '0')}-${picked.day.toString().padLeft(2, '0')}';
                    }
                  }),
                  const SizedBox(width: 8),
                  TDButton(text: '查询范围', size: TDButtonSize.small, type: TDButtonType.outline, onTap: _loadRange),
                ],
                ]),
              ),
          ...
          Expanded(
            child: loading
                ...
                : ListView(children: [
                    for (int i = 0; i < viewItems.length; i++) _buildOrderRow(i),
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      ...
      if (q.isEmpty) {
        ...
        viewItems = items.where((m) {
          final id = (m['product_id'] ?? m['id'] ?? '').toString().toLowerCase();
          final nm = (m['product_name'] ?? m['name'] ?? '').toString().toLowerCase();
  ...
  void _applySortOrder() {
    ...
    for (int i = 0; i < viewItems.length; i++) {
      final m = viewItems[i];
      final idx = _paixuIndex((m['product_name'] ?? m['name'] ?? '').toString());
    ...
    viewItems = enriched.map((e) => (e['item'] as Map<String, dynamic>)).toList();
    statusText = '共 ${viewItems.length} 条记录';
  }
  ...
  Future<void> _uploadSortFile() async {
    try {
      ...
      await ApiClient.post<Map<String, dynamic>>('/upload/sort-file', data: form);
    ...
    await showDialog(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          actions: [
            TDButton(text: '取消', type: TDButtonType.text, onTap: () => Navigator.of(ctx).pop()),
            TDButton(
              text: '保存',
              type: TDButtonType.outline,
              onTap: () async {
                try {
                  await ApiClient.post<Map<String, dynamic>>('/sort-order', data: {'text': ctrl.text});
                  await _loadSortOrder();
  ...
  Future<void> _importExcel() async {
    try {
      ...
      try {
        ...
        rowsStr = rows.map((r) => r.map((c) => (c?.value?.toString() ?? '').trim()).toList()).toList();
      } catch (_) {
        ...
        rowsStr = table.rows.map((r) => r.map((c) => (c?.toString() ?? '').trim()).toList()).toList();
      }

      final header = rowsStr.first.map((h) => h.toLowerCase()).toList();
      int idxId = header.indexWhere((h) => h.contains('编号') || h.contains('id'));
      int idxName = header.indexWhere((h) => h.contains('名称') || h.contains('品名') || h.contains('name'));
      int idxSpec = header.indexWhere((h) => h.contains('规格') || h.contains('spec'));
      int idxQty = header.indexWhere((h) => h.contains('库存') || h.contains('数量') || h.contains('qty') || h.contains('quantity'));
      int idxUnit = header.indexWhere((h) => h.contains('订货单位') || h.contains('单位') || h.contains('unit'));
      int idxLimit = header.indexWhere((h) => h.contains('订货限定') || h.contains('限定') || h.contains('地域') || h.contains('店'));
      ...
      for (int r = 1; r < rowsStr.length; r++) {
        ...
        String limit = idxLimit >= 0 && idxLimit < row.length ? row[idxLimit] : '';
        ...
        out.add({'rid': r - 1, 'product_id': pid, 'product_name': pname, 'spec': spec, 'quantity': qty, 'unit': unit, 'limit': limit, 'upb': upb, 'isBoxUnit': isBox, 'qtyVal': qtyVal, 'initBox': initBox});
      }
      items = out;
      ...
      // save to selected date if provided
      final date = chooseDateCtrl.text.trim();
      if (date.isNotEmpty) {
        await ApiClient.post<Map<String, dynamic>>('/order/items', data: {'items': items}, query: {'date': date});
      } else {
        await ApiClient.post<Map<String, dynamic>>('/order/items', data: {'items': items});
      }
      if (!mounted) return;
  ...
  Widget _buildOrderRow(int idx) {
    ...
    final upb = (m['upb'] ?? 0) is int ? (m['upb'] as int) : int.tryParse((m['upb'] ?? '0').toString()) ?? 0;
    final isBox = m['isBoxUnit'] == true;
    final ridKey = rid;
    final qtyCtrl = _qtyCtrls.putIfAbsent(ridKey, () => TextEditingController(text: (m['qtyVal'] ?? '').toString()));
    final unit = (m['unit'] ?? '').toString();
    final limit = (m['limit'] ?? '').toString();
    final qtyView = isBox ? (m['initBox'] ?? '').toString() : (m['qtyVal'] ?? '').toString();
      ...
      child: Column(children: [
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('${m['product_name'] ?? ''}', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
            Row(children: [
              Expanded(child: Text('编号: ${m['product_id'] ?? ''}  规格: ${m['spec'] ?? ''}', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.grey))),
              TextButton(onPressed: () { setState(() { if (detailExpanded.contains(rid)) { detailExpanded.remove(rid); } else { detailExpanded.add(rid); } }); }, child: Text(detailExpanded.contains(rid) ? '收起' : '查看更多')),
            ]),
            const SizedBox(height: 6),
              ...
              if (unit.isNotEmpty)
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.inventory_2, size: 14, color: Color(0xFF1E88E5)),
                    const SizedBox(width: 4),
                    Text('单位: $unit', style: const TextStyle(color: Color(0xFF1E88E5))),
                  ]),
                ),
              if (limit.isNotEmpty)
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.flag, size: 14, color: Color(0xFFFB8C00)),
                    const SizedBox(width: 4),
                    Text('限定: $limit', style: const TextStyle(color: Color(0xFFFB8C00))),
                  ]),
                ),
            ]),
            if (detailExpanded.contains(rid)) ...[
              const SizedBox(height: 8),
              Row(children: [
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFE8F5E9), borderRadius: BorderRadius.circular(6)), child: Text('每箱: ${upb > 0 ? upb.toString() : '-'}', style: const TextStyle(color: Color(0xFF2E7D32))))
              ])
            ]
          ]),
        ),
      Padding(
        ...
        child: Row(children: [
          Expanded(
            child: TDInput(
              ...
              onChanged: (val) {
                ...
                m['qtyVal'] = s;
                final boxVal = isBox && upb > 0 ? _computeBoxes(s, upb) : s;
                m['initBox'] = boxVal;
                setState(() {});
              },
            ),
          ...
          _ConfirmToggle(
            confirmed: confirmed,
            onToggle: () {
              confirmationStatus[rid] = !(confirmationStatus[rid] ?? false);
              setState(() {});
            },
          ),
        ]),
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
          child: Row(children: [
            Text(isBox ? '订货数量(箱)' : '订货数量', style: const TextStyle(color: Colors.grey)),
            const SizedBox(width: 12),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(8)), child: Text(_formatCompact(qtyView), style: const TextStyle(color: Color(0xFF16A34A), fontWeight: FontWeight.w600)))
          ]),
        )
      ]),
    );
  ...
  String _normalizeInput(String s) {
    ...
    if (firstDot >= 0) {
      t = t.substring(0, firstDot + 1) + t.substring(firstDot + 1).replaceAll('.', '');
    }
  ...
  String _computeBoxes(String qtyStr, int unitsPerBox) {
    ...
    return boxes.toStringAsFixed(4).replaceFirst(RegExp(r'0+$'), '').replaceFirst(RegExp(r'\.$'), '');
  }
...
// -- end of _OrderPageState methods --
  ...
  Future<void> _copyOrderColumn() async {
    final lines = viewItems.map((m) {
      final isBox = m['isBoxUnit'] == true;
      final s = isBox ? (m['initBox'] ?? '').toString() : (m['qtyVal'] ?? '').toString();
      return s;
  ...
  Future<void> _saveOrder() async {
    try {
      ...
      await ApiClient.post<Map<String, dynamic>>('/order/items', data: {'items': payloadItems});
      final today = DateTime.now();
      final key = 'history_${today.year.toString().padLeft(4, '0')}-${today.month.toString().padLeft(2, '0')}-${today.day.toString().padLeft(2, '0')}';
      final record = {
        'displayData': viewItems,
        'isSorted': isSorted,
        'confirmationStatus': confirmationStatus.entries.map((e) => [e.key, e.value]).toList(),
      };
      await ApiClient.post<Map<String, dynamic>>('/order/save', data: {'key': key, 'data': record});
      if (!mounted) return;
  ...
  Future<void> _loadByDate() async {
    ...
    setState(() => loading = true);
    try {
      final resp = await ApiClient.get<Map<String, dynamic>>('/order/items', query: {'date': date});
      final arr = (resp.data?['items'] as List?) ?? [];
      items = arr.map((e) => (e as Map).map((k, v) => MapEntry(k.toString(), v))).cast<Map<String, dynamic>>().toList();
      viewItems = _normalizeItems(items);
  ...
  Future<void> _loadRange() async {
    ...
    setState(() => loading = true);
    try {
      final resp = await ApiClient.get<Map<String, dynamic>>('/order/items-range', query: {'start': s, 'end': e});
      final data = (resp.data?['data'] as List?) ?? [];
      // flatten items, but keep latest date first
  ...
  List<Map<String, dynamic>> _normalizeItems(List<Map<String, dynamic>> src) {
    ...
    for (int i = 0; i < src.length; i++) {
      ...
      m['isBoxUnit'] = isBox;
      m['initBox'] = isBox && upb > 0 ? _computeBoxes(m['qtyVal'], upb) : m['qtyVal'];
      out.add(m);
    }
    return out;
  }
...
class _ConfirmToggleState extends State<_ConfirmToggle> {
  ...
  Widget build(BuildContext context) {
    ...
    final border = confirmed ? const Color(0xFF2E7D32) : const Color(0xFF9CA3AF);
